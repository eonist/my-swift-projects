...
class TriangleMath {
    ...
    */
    public static function inCenter(a:Number, b:Number, c:Number,p1:Point,p2:Point,p3:Point):Point {
    var x:Number = ((a*p1.x)+(b*p2.x)+(c*p3.x))/(a+b+c);
    var y:Number = ((a*p1.y)+(b*p2.y)+(c*p3.y))/(a+b+c);
