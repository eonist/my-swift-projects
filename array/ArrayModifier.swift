...
class ArrayModifier{
    ...
    */
     ...
     */
    class func removeManyByKey<T>(array:[Dictionary<T,T>],_ many:Array<String>,_ key:String) -> [Dictionary<T,T>] {
        for (var i : Int = 0; i < array.count; i++) if(ArrayParser.indx(many.(array[i][key]) != -1) { array.splice2(i--,1) }
        return array
    }
    /**
