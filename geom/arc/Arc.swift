...
class Arc:IArc{
     ...
     * TODO: the reason Arc4 exists is because it can work with axises that are 0, if you want to Make Arc3 the primary arc class again you need to make sure it can also work with axis that are 0, the benefit of Ar3 is that its absolute precis while editing the arc instance start and end positions.
     * TODO: a largeflag two that indicates wheter the start point is on the left or right side when one of the axises is 0 ould be a solution to the problem with 0 axises
