class StringAsserter{
	 ...
	 */
	static func isWrappedWith(string:String, _ char:Character)->Bool{
	 ...
	 */	
	static func beginsWith(string:String, _ prefix:String)->Bool{
     ...
     */
    static func endWith(str:String, _ suffix:String)->Bool{
     ...
     */
    static func contains(a:String, _ b:String)->Bool{
        return a.rangeOfString(b) != nil
    }
    /**
     * Asserts if a string is a digit (10, 20px, -20px, 0.2px, -.2, 20%, 0.2)
     */
    static func digit(string:String)->Bool{
        return RegExp.test(string, "^-?\\d*?\\.?\\d*?(px)?$")
    }
    /**
     *
     */
    static func metric(string:String) -> Bool{
        return RegExp.test(string, "^-?\\d*?\\.?\\d*?(ems|%)?$")
    }
    /**
     *
     */
    static func color(string:String) -> Bool {
        return RegExp.test(string,"^#?([a-fA-F0-9]{3}){1,2}$")
    }
    /**
     *
     */
    static func webColor(string:String)->Bool {
     ...
     */
    static func lowerCase(string:String)->Bool {
        return string == string.lowercaseString
    }
	/**
	 * Asserts if PARAM: value is a percentage
	 */
	static func percentage(value:String)->Bool{
		return value.test(".*?(?=%)")
	}
}