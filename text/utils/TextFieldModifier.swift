...
class TextFieldModifier {
    ...
    class func applyTextFormat(textField:TextField,_ textFormat:TextFormat){
        ...
        textField.backgroundColor = textFormat.backgroundColor//textFormat.background ? textFormat.backgroundColor : NSColor.clearColor()
        textField.drawsBackground = textFormat.background//<--this is a temp fix so that one can add or remove a background while the app is running, this should ideally be done automatically when the text is re-rendered, more research needed
