...
class EventSender:IEventSender {
    var eventCall:((Event) -> ())? {
        return {
            (event:Event) -> Void in Swift.print("\(self.dynamicType)  event handler missing,  event.type: \(event.type) event.origin: \(event.origin)" )//Empty call so that if no event handler is attached it doesnt throw an error
        }
