...
class Spring<T:NumberKind>:BaseAnimation {
    ...
    init(_ animatable:Animatable, _ callBack:@escaping FrameTick,  _ config:(spring:T, friction:T) , _ initVals:(value:T,targetValue:T,velocity:T), _ stopAssert:@escaping StopAssert? = nil) {
        self.value = initVals.value/*Set the init value*/
        self.targetValue = initVals.targetValue
        self.velocity = initVals.velocity
        self.callBack = callBack
        self.config = config
        self.stopAssert = stopAssert ?? defaultStopAssert
