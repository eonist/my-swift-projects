...
extension NSView {
     ...
     */
    func hitTestPointToView(locationInWindow:NSPoint, _ toView:NSView? = nil)->Bool{
        let mousePos:NSPoint = convertPoint(locationInWindow, toView: toView)
        return NSPointInRect(mousePos, frame)
    }
    func hitTestPointFromView(locationInWindow:NSPoint, _ fromView:NSView? = nil)->Bool{
